...
Database.logger = Logger.new(config['wednesday_log'])
...
users.each do |u|
  ...
  reco_ids = u[1].split(",")[1]
  recoed_users = User.find_all_by_id(reco_ids)
  next if recoed_users.empty?
  UserMailer.delay.send_mail( { :template => "wednesday_reminders", :subject => "As promised, your recommendations this Wednesday" }, [ user, recoed_users ].flatten )
end
